...
double grade_of_position(int(&array)[24], short depht, short enterPlayer) {
    ...
    for (int i = 1; i < 7; i++) {
        for (int j = 1; j < 7; j++) {
            int number_of_moves = 0;
            if (enterPlayer == 1) {
                number_of_moves = get_all_possible_moves(array, i, j, 1);
            }
            else {
                number_of_moves = get_all_possible_moves(array, i, j, 2);
            }
...
void AIPlayer::OnTurnEnter()
{
    ...
    case (AiDifficulty::Hard):
        printf("assssssssssssssssssssssssssssssssssssssssssssssssssss\n");
        int board[24];
        for (int i = 0; i < 24; i++) {
